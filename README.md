# Smarkets Coding Challenge

### Intro

I created this MVP of a React - Node app in 2 hours for the Smarkets coding challenge.

It uses their frontend API to retrieve the most popular events, and then get more details about each of those events.

For MVP the interface is extremely minimal and unstyled, but this skeleton would make a good foundation for further iterations.



### Next steps

If I were to do further work, the next steps might look something like:

- Adding more details about each event.


- Writing unit tests using [Jest](https://facebook.github.io/jest/).
- Styling the app, using CSS in the first instance, but potentially bringing in a framework like [Ant Design](https://ant.design/) later.
- Implementing some environment-detection logic in the server, so the app could be deployed properly in a production-like environment.
- Setting up continuous integration using e.g. Travis or CircleCI.
...
```
git clone https://github.com/mattdean1/smarkets mattdean_smarkets_challenge

cd mattdean_smarkets_challenge
